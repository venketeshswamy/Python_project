...
class ConfigManager:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"'{self.config_path}' not found or invalid. Creating default config.")
        ...
        for i, file_path in enumerate(self.file_paths):
            ...
            try:
                ...
                while os.path.exists(imported_path):
                    ...
                    imported_path = os.path.join(self.import_dir, f"{name}_{counter}{ext}")
                    counter += 1
    ...
    def run(self):
        try:
            ...
            if pixmap and not pixmap.isNull():
                ...
                self.error.emit(f"Failed to render page {self.page_num + 1} from {os.path.basename(self.file_path)}.")
...
class OcrWorker(QThread):
    ...
    def __init__(self, tasks: List[Dict], config: Dict[str, Any]):
        ...
        if WINDOWS_OCR_AVAILABLE:
            try:
                self.win_ocr_engine = OcrEngine.try_create_from_language(Language(config.get("windows_ocr_lang", "en-US")))
            except Exception as e:
    ...
    def run(self):
        ...
        for i, task in enumerate(self.tasks):
            ...
            self.progress.emit(int((i / total_tasks) * 100), f"Processing snip '{task['snip_name']}' on page {task['page_num']+1}...")
            ...
            try:
                ...
                if ocr_engine == 'pdf_native':
                    ...
                    text = self._run_windows_ocr(file_path, page_num, snip_rect)
                else:
                    self.error.emit(f"Unsupported or unavailable OCR engine: {ocr_engine}")
                    continue
                ...
                self.error.emit(f"OCR failed for snip '{task.get('snip_name', 'Unknown')}': {e}")
    ...
    def _run_tesseract(self, file_path: str, page_num: int, snip_rect: QRectF) -> str:
        ...
        try:
            ...
            self.error.emit("Tesseract not found. Check 'tesseract_cmd' in config.json.")
            return ""
        except Exception as e:
    ...
    async def _perform_win_ocr(self, img_bytes: bytes, width: int, height: int) -> str:
        ...
        try:
            software_bitmap = SoftwareBitmap(BitmapPixelFormat.RGBA8, width, height)
...
class SnipItem(QGraphicsRectItem):
    ...
    def interactive_resize(self, mouse_pos: QPointF):
        ...
        rect = self.rect()
        if self.mouse_press_rect is None: return
    ...
    def update_handles(self):
        ...
        self.handles[3] = QRectF(r.right() - s, r.center().y() - s/2, s, s) # Right
        self.handles[4] = QRectF(r.right() - s, r.bottom() - s, s, s) # Bottom-right
        self.handles[5] = QRectF(r.center().x() - s/2, r.bottom() - s, s, s) # Bottom
    ...
    def paint(self, painter: QPainter, option, widget=None):
        ...
        painter.setPen(pen)
        painter.setBrush(QBrush(QColor(self.color.red(), self.color.green(), self.color.blue(), 50)))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # --- Data Models ---
        self.imported_files: Dict[str, Dict] = {} # {imported_path: {orig_path, num_pages}}
        self.snips: Dict[str, Dict] = {} # {snip_name: {rect: QRectF, color: QColor, item: SnipItem}}
    ...
    def _create_actions(self):
        ...
        # File Actions
        self.import_files_action = QAction(QIcon.fromTheme("document-open"), "&Import Files...", self)
        self.import_folder_action = QAction(QIcon.fromTheme("folder-open"), "Import &Folder...", self)
        self.save_session_action = QAction(QIcon.fromTheme("document-save-as"), "&Save Session As...", self)
        self.load_session_action = QAction(QIcon.fromTheme("document-open"), "&Load Session...", self)
        self.export_csv_action = QAction(QIcon.fromTheme("document-save"), "Export to &CSV...", self)
        self.export_excel_action = QAction(QIcon.fromTheme("x-office-spreadsheet"), "Export to E&xcel...", self)
        ...
        self.prefs_action = QAction(QIcon.fromTheme("preferences-system"), "Application &Preferences...", self)
        ...
        self.fit_page_action = QAction(QIcon.fromTheme("zoom-fit-best"), "&Fit to Page", self)
        self.fit_width_action = QAction(QIcon.fromTheme("zoom-fit-width"), "Fit to &Width", self)
        self.bw_toggle_action = QAction("Display Black & White", self, checkable=True)
    ...
    def _create_toolbar(self):
        ...
        if index != -1: self.ocr_engine_combo.setCurrentIndex(index)
        ...
        self.run_ocr_button = QPushButton(QIcon.fromTheme("media-playback-start"), "Run OCR")
        self.stop_ocr_button = QPushButton(QIcon.fromTheme("media-playback-stop"), "Stop OCR")
    ...
    def _create_left_pane(self):
        ...
        self.files_list_widget.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        ...
        self.snips_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
        self.snips_table.verticalHeader().setVisible(False)
        self.snips_table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows)
    ...
    def _create_center_pane(self):
        ...
        # Info label
        info_label = QLabel("<b>Hint:</b> Hold <b>Shift</b> and drag to draw a snip. Select snips to move/resize.")
    ...
    def _create_right_pane(self):
        ...
        self.results_table.horizontalHeader().setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
    ...
    def _connect_signals(self):
        ...
        self.export_csv_action.triggered.connect(lambda: self._export_results('csv'))
        self.export_excel_action.triggered.connect(lambda: self._export_results('excel'))
        ...
        # View Menu
        self.zoom_in_action.triggered.connect(lambda: self.graphics_view.scale(1.2, 1.2))
        self.zoom_out_action.triggered.connect(lambda: self.graphics_view.scale(1/1.2, 1/1.2))
        self.fit_page_action.triggered.connect(self.graphics_view.fit_to_page)
        self.fit_width_action.triggered.connect(self.graphics_view.fit_to_width)
        ...
        # Left Pane
        self.files_list_widget.currentItemChanged.connect(self._on_file_selected)
        self.files_list_widget.customContextMenuRequested.connect(self._show_files_list_context_menu)
        self.snips_table.itemDoubleClicked.connect(self._rename_snip)

        # Center Pane
        self.first_page_button.clicked.connect(lambda: self._navigate_to_page(0))
        self.prev_page_button.clicked.connect(lambda: self._navigate_to_page(self.current_page_num - 1))
        self.next_page_button.clicked.connect(lambda: self._navigate_to_page(self.current_page_num + 1))
    ...
    def dropEvent(self, event):
        ...
        if supported_files:
            ...
            QMessageBox.warning(self, "Unsupported Files", "None of the dropped files are of a supported format.")
    ...
    def _update_status_bar(self):
        ...
        if self.current_file_path and self.current_file_path in self.imported_files:
            ...
            self.status_file_label.setText(f"File: {os.path.basename(self.current_file_path)}")
            self.status_page_label.setText(f"Page {self.current_page_num + 1} of {file_info['num_pages']}")
        else:
    ...
    def _prompt_import_folder(self):
        ...
        if folder_path:
            ...
            if file_paths:
                ...
                QMessageBox.information(self, "No Files Found", "No supported files were found in the selected folder.")

    def _start_file_import(self, file_paths: List[str]):
        """Starts the file import worker thread."""
        if self.import_worker and self.import_worker.isRunning():
            QMessageBox.warning(self, "Busy", "An import operation is already in progress.")
            return
    ...
    def _handle_imported_file(self, original_path: str, imported_path: str):
        ...
        try:
            ...
            self._show_error_message(f"Could not open or process file: {os.path.basename(imported_path)}\n\nError: {e}")
    ...
    def _render_current_page(self):
        ...
        self.render_worker.finished.connect(lambda: self.status_bar.clearMessage())
    ...
    def _apply_bw_filter(self):
        ...
        pixmap_item = next((item for item in self.graphics_scene.items() if isinstance(item, QGraphicsPixmapItem)), None)
        if pixmap_item:
    ...
    def _add_new_snip(self, rect: QRectF):
        ...
        snip_item.geometry_changed.connect(lambda: self._on_snip_geometry_changed(snip_name))
    ...
    def _update_snips_table(self):
        ...
        for name, data in self.snips.items():
            ...
            name_item.setFlags(name_item.flags() & ~Qt.ItemFlag.ItemIsEditable) # Read-only here
    ...
    def _rename_snip(self, item: QTableWidgetItem):
        ...
        new_name, ok = QInputDialog.getText(self, "Rename Snip", "Enter new name:", text=old_name)
        
        if ok and new_name and new_name != old_name:
            if new_name in self.snips:
                QMessageBox.warning(self, "Name Exists", "A snip with this name already exists.")
                return
            ...
            if not self.ocr_results.empty and old_name in self.ocr_results.columns:
                self.ocr_results.rename(columns={old_name: new_name}, inplace=True)
    ...
    def _on_snip_geometry_changed(self, snip_name: str):
        ...
        if snip_name in self.snips:
            ...
            self.snips[snip_name]['rect'] = self._normalize_rect(new_scene_rect)
    ...
    def _update_snips_on_canvas(self):
        ...
        for name, data in self.snips.items():
            ...
            snip_item.geometry_changed.connect(lambda n=name: self._on_snip_geometry_changed(n))
    ...
    def _run_ocr_task(self):
        ...
        if self.ocr_worker and self.ocr_worker.isRunning():
            QMessageBox.warning(self, "Busy", "An OCR task is already running.")
            return

        if not self.snips:
            QMessageBox.warning(self, "No Snips", "Please define at least one snip before running OCR.")
            return

        tasks = self._prepare_ocr_tasks()
        if not tasks:
            QMessageBox.warning(self, "No Tasks", "Could not generate any OCR tasks. Check your file and page range selections.")
            return
    ...
    def _prepare_ocr_tasks(self) -> List[Dict]:
        ...
        if snip_mode == 'single':
            ...
            if not selected_files:
                QMessageBox.warning(self, "No Files Selected", "Please select one or more files from the list for Template mode.")
                return []
            
            page_indices = self._parse_page_range(self.page_range_edit.text())
            if not page_indices:
                QMessageBox.warning(self, "Invalid Page Range", "Please enter a valid page range (e.g., 1, 3-5).")
                return []
        ...
        if self.ocr_results.empty:
            ...
            if not match.empty:
                ...
                if result['snip_name'] not in self.ocr_results.columns:
                    self.ocr_results[result['snip_name']] = "" # Add new column
                self.ocr_results.loc[index, result['snip_name']] = result['text']
            else:
                # Append new row, merging columns
                self.ocr_results = pd.concat([self.ocr_results, new_row], ignore_index=True)
    ...
    def _show_files_list_context_menu(self, pos: QPoint):
        ...
        if not self.files_list_widget.selectedItems(): return
    ...
    def _delete_selected_files(self):
        ...
        for item in items_to_delete:
            ...
            if not self.ocr_results.empty:
                self.ocr_results = self.ocr_results[self.ocr_results['Source File'] != os.path.basename(file_path)]
            # Optionally, delete the file from imports_dir
            if os.path.exists(file_path):
                try:
                    os.remove(file_path)
                except OSError as e:
                    self._show_error_message(f"Could not delete file from workspace: {e}")
    ...
    def _show_results_table_context_menu(self, pos: QPoint):
        ...
        if action == copy_cell_action:
            ...
            row_text = "\t".join([self.results_table.item(item.row(), c).text() for c in range(self.results_table.columnCount())])
    ...
    def _go_to_snip_location(self, row, col):
        ...
        if snip_item:
            for item in self.graphics_scene.items(): item.setSelected(False)
    ...
    def _show_header_context_menu(self, pos: QPoint):
        ...
        if action == filter_action:
            if not self.ocr_results.empty and col_name in self.ocr_results.columns:
                unique_values = self.ocr_results[col_name].astype(str).unique().tolist()
    ...
    def _save_session(self):
        ...
        if not self.imported_files:
            QMessageBox.warning(self, "Empty Session", "There are no imported files to save.")
            return
        ...
        try:
            with zipfile.ZipFile(file_path, 'w', zipfile.ZIP_DEFLATED) as zf:
                ...
                if not self.ocr_results.empty:
                    zf.writestr('results.csv', self.ocr_results.to_csv(index=False))

                # 2. Save imported files
                for imported_path in self.imported_files.keys():
                    if os.path.exists(imported_path):
                        zf.write(imported_path, arcname=os.path.basename(imported_path))
            
            self.status_bar.showMessage(f"Session saved to {os.path.basename(file_path)}", 5000)
    ...
    def _load_session(self):
        ...
        try:
            ...
            self.status_bar.showMessage(f"Session loaded from {os.path.basename(file_path)}", 5000)
    ...
    def _export_results(self, format: str):
        ...
        if self.ocr_results.empty:
            QMessageBox.warning(self, "No Results", "There are no OCR results to export.")
            return
            
        ext, file_filter = ('.csv', "CSV Files (*.csv)") if format == 'csv' else ('.xlsx', "Excel Files (*.xlsx)")
        
        file_path, _ = QFileDialog.getSaveFileName(self, f"Export to {format.upper()}", self.config.get("exports_dir"), file_filter)
        ...
        try:
            if format == 'csv':
                ...
                try:
                    import openpyxl
                    self.ocr_results.to_excel(file_path, index=False, engine='openpyxl')
                except ImportError:
                    self._show_error_message("The 'openpyxl' library is required to export to Excel.\nPlease install it using: pip install openpyxl")
                    return
            
            self.status_bar.showMessage(f"Results exported to {os.path.basename(file_path)}", 5000)
